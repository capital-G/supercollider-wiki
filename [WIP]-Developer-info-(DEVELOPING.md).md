...
Document useful information for developers and maintainers of SuperCollider.

This document is a WIP, feel free to add to it!

When this is added into the main repo, make sure to change the link in the CONTRIBUTING file.

TODO: document other notable sc-org repositories
...
- **`external_libraries`**: C and C++ third-party libraries
  ...
  - **`boost_sync`**: contains [Boost::sync](https://github.com/boostorg/sync), a synchronization lib which was never integrated into Boost proper (see [this comment](http://new-supercollider-mailing-lists-forums-use-these.2681727.n2.nabble.com/RE-boost-submodules-tp7630455p7630457.html)).
  - **`hidapi`**: [HID API](http://www.signal11.us/oss/hidapi/) project, which `sclang` uses for HID capability. Our fork is significantly patched for cross-platform compatibility.
  - **`icu`**: IBM's [ICU](http://site.icu-project.org/) library (used for building Boost libraries??)
  ...
  - **`libsndfile`**: header for [`libsndfile`](http://www.mega-nerd.com/libsndfile/), for working with audio file formats. Used by `sclang` and the servers
  ...
  - **`portaudio_sc_org`**: SC's fork of [PortAudio](http://www.portaudio.com/), which has been modded for cross-platform work
...
- **`testsuite`**: test files
  - **`classlibrary`**: SuperCollider tests for the core class library
  - **`sclang`**: tests for `sclang` (TODO explain)
...
- **`build_sclang_cfg.in`**: config file used for the special `SC_DOC_RENDER` build target (TODO: document this target)
...
- **`travis_test_run_proto.json`**: config file used by qpm to run tests in CI (TODO: document qpm)
